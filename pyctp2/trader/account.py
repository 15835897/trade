...
class Account(object):
    ...
    def initialize(self):
        self.spi = TraderSpiDelegate(self._ports_info.broker.encode(encoding='utf-8', errors = 'strict'),
                                     self._ports_info.investor.encode(encoding='utf-8', errors = 'strict'),
                                     self._ports_info.passwd.encode(encoding='utf-8', errors = 'strict')
                                     )
        ...
        self.api = self.spi
        path=INTERNAL_PATH+'/'+self._ports_info.name
        szpath=path.encode(encoding='utf-8', errors = 'strict')
        self.api.Create(szpath)
        print('path:'+path)
